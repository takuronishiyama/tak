...
GP.race = (function () {
  ...
  const SAY = {
    ...
    aqua: [
      ...
      '{A} が水たまりを踏んだ！ {T} では掻き出せない！',
      '{A}、まっすぐ走らない！ {C} の入口で舵が利いていない！',
      '{A} が流れた！ 雨の {C} を {T} で行くのは無理だ！'
    ],
    aquaOut: [
      '{A} が水に乗ったままバリアへ！ {T} のまま走り続けた代償だ…',
      '{A}、ハイドロプレーニングでコースアウト。雨の {C} に溝のないタイヤでは…',
      '{A} が {C} の外へ消えた。ピットに入れるべきだった…',
      '{A}、水に乗ったまま止まれない——そのままグラベルへ…',
      '{A} が {C} で姿勢を失い、戻ってこられなかった…'
    ],
    spinOut: [
      ...
      '{A} が {C} で止まりきれず、バリアに当てた…',
      '{A}、リアが出たまま戻らない。ここでレースが終わった…'
    ],
    /* 機械の故障でリタイア */
    dnfMech: [
      '{A} が{B}でリタイア…',
    ...
    pit: [
      ...
      '{A} ボックス！ 4本そろって {B}、ジャッキが下りた（静止{S}秒・計{P}秒）',
      '{A} が滑り込んできた。{B}に替えて、隙間を狙って送り出す（計{P}秒）',
      '{A}、ここでタイヤを替える。{B}で勝負をかけるつもりだ（静止{S}秒）',
      '{A} ピットレーンへ。{B}を履いて、残りの組み立てが変わる（計{P}秒）'
    ],
    /* 手間取ったピット */
    pitSlow: [
      ...
      '{A}、左リアが渋い！ ガンが外れない（静止{S}秒・計{P}秒）',
      '{A} ピットイン——ジャッキが引っかかった！ {B}（静止{S}秒・計{P}秒）',
      '{A}、合図が出ない！ 数秒がそのまま順位になる（静止{S}秒・計{P}秒）'
    ],
    /* ファステストラップを更新した */
    fastest: [
      ...
      '{A} が全体ベスト！ この周だけ、別のレースを走っているようだ',
      '{A}、区間すべてが紫！ 誰もついていけない',
      '{A} がタイムを叩き出した。タイヤはまだ生きている',
      '{A}、ここに来てファステスト。まだ先があるということか'
    ],
    /* 表彰台圏に入った／落ちた */
    intoPodium: [
      '{A} が3位に浮上！ 表彰台が見えてきた！',
      ...
      '{A} が3番手へ！ ピットウォールが立ち上がった',
      '{A}、表彰台圏内に浮上！ ここまで長かった',
      '{A} が {P}位。残りを守りきれば、壇上に立てる'
    ],
    outPodium: [
      '{A} が表彰台圏から落ちた…{P}位',
      '{A}、譲るしかなかった。表彰台が遠ざかる…',
      '{A} が {P}位へ後退。ここから立て直せるか',
      '{A}、抜かれた。表彰台の景色が遠のいていく…',
      '{A} が {P}位に落ちる。守りきれなかった',
      '{A}、ここで順位を明け渡す。悔しい {P}位だ'
    ],
    /* 入賞圏に入った */
    intoPoints: [
      ...
      '{A}、{P}位でポイント圏へ！ チームにとって大きな1歩だ',
      '{A} がトップ10に入った！ ガレージから歓声が上がる',
      '{A}、ここで {P}位。地味だが、これが積み重なる'
    ],
    /* チームメイトどうしの争い */
    teammate: [
      ...
      '同じ色が2台、並んだまま {C} へ！ ピットウォールは何も言えない',
      '{A} と {B}——ここでぶつかれば、失うのはチームだ',
      'チーム内の順位争い。{A} が {B} に並びかけた！'
    ],
    /* 最終ラップ突入 */
    finalLap: [
      'ファイナルラップ！ 最後の1周が始まった！',
      ...
      '残り1周。全員がすべてを出しきる！',
      '最終ラップ！ ここまでの積み重ねが、あと1周で決まる',
      'ファイナルラップに入った。タイヤも燃料も、もう気にしない',
      '残り1周——ここからは度胸の勝負だ！'
    ],
    /* 母国グランプリ */
    home: [
      ...
      'ホームストレートの声援が {A} を押している',
      '{A} の母国グランプリ。ここで走る意味を、本人がいちばん知っている',
      'スタンドが {A} の名を呼んでいる。この一戦だけは特別だ'
    ],
    ...
    pitCheap: [
      ...
      '{A}、ここしかないタイミングでボックス！ {B}（ロード{L}秒・計{P}秒）',
      '{A} が一番乗りでピットへ！ 隊列のうちに済ませた（{B}・計{P}秒）'
    ]
  };
  ...
  function simulate(g, trackIndex, strategy, special, pre) {
    ...
    /* ---- 金曜の使い道を、日曜ぶんに差し替える ----
       entries は予選で組んだものを使い回すので、
       そこには「土曜に効くぶん」が入ったままになっている。
       セットアップを詰めた週末は予選で速く、ロングランを走った週末は
       日曜で速い——それを成立させるために、ここで掛け直す。
       グリッドはもう決まっているので、並びには触らない          */
    if (strategy && strategy.setupR && strategy.setup) {
      const k2 = strategy.setupR / strategy.setup;
      if (Math.abs(k2 - 1) > 1e-6) {
        entries.forEach(e => {
          if (!e.isPlayer) return;
          e.pmul *= k2;
          e.perf *= k2;
          if (e.secPerf) e.secPerf = e.secPerf.map(v => v * k2);
        });
      }
    }
    const laps = Math.max(4, Math.round(track.laps * (special ? special.lapMul : 1)));
    ...
    entries.forEach(e => {
      ...
      /* 週の途中で「雨の支度をする」を選んでいたら、そのぶん濡れた路面に強い。
         使い切りなので、走り出したところで降ろす（消費は simulate の末尾）  */
      if (e.isPlayer && (g.wetReady || 0) > 0) e.wetSkill = S.clamp(e.wetSkill + 0.18, 0, 1);
    ...
    for (let lap = 1; lap <= laps; lap++) {
      ...
      if (scPending && !scDone) {
        ...
        run.forEach(e => {
          ...
          /* 「早く入ると最後の区間が伸び、もう一度止まることになる」
             という危険をここで値付けてみたが、同じレースを
             二回走らせて比べたところ、秤の当たりが落ちた。
             （「入れ」と「待て」の開き 3.76 → 2.93 順位、
               秤に従ったときの取り分 +0.70 → +0.45 順位）。
             隊列で入る車は、どうせ次のピットが近い車なので、
             その危険は②の「捨てる周」がすでに数えている。
             二重に数えると、入るべきときに入らなくなる。        */
      ...
      /* ---- 隊列は一列のまま ----
         セーフティカーの後ろでは、追い越しはできない。
         SC中も1周のタイムに揺れを入れているので、
         0.02秒差で並んでいる2台が、ただの揺れで入れ替わることがあった。
         前の周の並びをそのまま守る。
         ただしピットに入った車が下がるのは正しいので、そこは触らない  */
      const scHold = scLaps > 0 && lap >= scFrom && lap < scFrom + scLaps;
      if (scHold && lap > 1 && posHistory.length) {
        const prevIds = posHistory[posHistory.length - 1];
        const held = order.filter(e => !e.dnf && e.cum[lap - 1] != null &&
                                       e.pitPlan.indexOf(lap) < 0 &&
                                       prevIds.indexOf(e.id) >= 0)
          .sort((a, b) => prevIds.indexOf(a.id) - prevIds.indexOf(b.id));
        for (let i = 1; i < held.length; i++) {
          const ahead = held[i - 1], me = held[i];
          // 前にいた車より前へ出ていたら、そのすぐ後ろへ戻す
          if (me.cum[lap - 1] <= ahead.cum[lap - 1]) {
            me.cum[lap - 1] = ahead.cum[lap - 1] + 0.08;
          }
        }
      }
      ...
      if (lap > 1) {
        newOrder.forEach((e, i) => {
          ...
          if (prevIdx >= 0 && prevIdx > i && e.isPlayer) {
            ...
            /* 順位が上がった理由を見てから書く。
               セーフティカー中は追い越せないし、
               相手がピットに入って下がっただけなら、抜いたわけではない。
               どちらも、抜いたと書かずに黙っておく（ピットは別に実況が出る） */
            const pitted = victim && victim.pitPlan.indexOf(lap) >= 0;
            if (victim && !scHold && !pitted) {
              events.push({ lap, type: 'pass', car: e, text: e.driver.name + ' が ' + victim.driver.name + ' をオーバーテイク！ → ' + (i + 1) + '位' });
            }
          }
        });
      }
    ...
    /* 週の途中で仕込んだぶんは、この1戦で使い切る */
    if (g.wetReady) g.wetReady = Math.max(0, g.wetReady - 1);
    if (g.simGain) g.simGain = Math.max(0, g.simGain - 1);
