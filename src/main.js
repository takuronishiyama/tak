...
(function () {
  ...
  function endWeek() {
    ...
    /* 製造方針が「予備も回す」なら、毎週すこしずつ組み上がる。
       思い出したときに買う一台ではなく、方針の結果として増える  */
    {
      const sp = S.tickSpare(g);
      if (sp && sp.done) {
        U.log(g, '🧰 予備が1台 組み上がった（いま ' + sp.now + ' / ' + D.SPARE.max +
          '）。壊した週末に載せ替えられる', 'good');
        U.toast('🧰 予備が組み上がった（' + sp.now + '台）', 'good');
      }
    }
    /* 煮詰めも、つなぎ込みと同じく部門の毎週の仕事。
       上限に届いたら、そこではじめてプレイヤーの判断が要る    */
    {
      const ai = S.autoImprove(g);
      if (ai) {
        g.lastImp = ai;
        if (ai.total >= 1.0) {
          U.log(g, '🏭 工房がパーツを煮詰めた（' +
        ...
        /* 上限に届いた瞬間だけは、はっきり知らせる。
           ここが「作るか、⚡イノベーションか」を決める場面   */
        if (ai.hitCap.length) {
          U.log(g, '🔩 ' + ai.hitCap.join('・') + ' が器の上限に届いた。' +
            'ここから先は、作り直すか、⚡イノベーションで器そのものを広げるか。' +
            '🏗️施設を伸ばしても器は広がる', 'warn');
          U.toast('🔩 ' + ai.hitCap[0] + ' が上限に届いた', 'warn');
        }
      }
    }
  ...
  function announceGen(up) {
    ...
    /* 仕込んであったぶんは、ここで新型の出発点になる。
       黙って消えると「バーが 0% に戻った」としか見えない      */
    if (up.advance > 0.05) {
      /* 日誌は素のテキストとして出す（ui.js が esc する）。
         ここに飾りのタグを混ぜると、そのまま文字として出てしまう  */
      U.log(g, '🌱 仕込んであった ' + up.stock + ' が、そのまま新型の出発点になった' +
               '（車体の各項目 +' + up.advance + '）。' +
               '仕込みはここで使い切ったので、また 0 から溜まります', 'good');
      U.pop('🌱 仕込み +' + up.advance, 'good');
    }
