/* =========================================================
   画面：車体と開発（設計室・工房・ガレージ・研究所・整備・練習）
...
GP.screens.dev = function (A) {
  ...
  function cmdCar() {
    ...
    /* 選ぶものを先に置く。
       ...
       輪の絵と成り立ちは、その判断の材料。
       横向きのスマホで測ったら、画面8.7枚ぶんスクロールしないと
       ボタンに届かなかった。順番が逆だった            */
    let body =
  ...
  function cmdDesign(tab) {
    ...
    let body =
      ...
      'それを形にするのは🏭工房、車に載せて作り込むのは🔧ガレージです。</p>' +
    ...
    if (desTab === 'idea') {
      ...
      /* ---- いまの規則を、どれだけ読めているか ----
         今季と来季の振り分けを決める材料なので、この札の上に置く。
         規則が変わった年は、プレイヤーも手探りをする（ライバルだけではない） */
      {
        const rd = S.myEraRead(g), fit = S.myEraFit(g);
        const since = S.regSince(g);
        const pct = Math.round((fit - 1) * 100);
        body += '<div class="erabox' + (fit >= 1.05 ? ' up' : fit <= 0.95 ? ' down' : '') + '">' +
          '<b>📜 いまの規則 ' + (since + 1) + '年目　' + rd.icon + ' ' + rd.name +
          '（開発の伸び ' + (pct >= 0 ? '+' : '') + pct + '%）</b>' +
    ...
    S.techList(g).forEach(t => {
      ...
      devRow['tec:' + t.key] = {
        ...
        lines: maxed ? [['これ以上', '上がりません']]
          : [['いまの効き', '<b>' + now + '</b>'],
             ...
             ['上がると', esc(t.eff)]],
        note: esc(t.desc) + (maxed ? '' : '　' + esc(t.note)) +
              '　一度ものにした技術は、' +
              'パーツを作り替えても失われません。',
      ...
      body += '<button class="pickbtn techrow' + (ok ? '' : ' cant') + '" data-k="tec:' + t.key + '">' +
        ...
        (maxed ? '<span class="devup">これ以上は上がりません</span>'
               : '<span class="devup">あと <b>' + left + '回</b> で Lv.' + (t.lv + 1) + '</span>') +
        '</small></span>' +
  ...
  function lineBoxHTML() {
    ...
    D.PART_CATS.forEach(c => {
      ...
      h += '<div class="linerow">' +
        ...
        '</small></span>' +
        '<span class="ln-eff">品質の底 <b>+' + ln.qual.toFixed(3) + '</b><br>' +
        '削り出しの精度 <b>+' + Math.round((ln.prec - 1) * 100) + '%</b><br>' +
