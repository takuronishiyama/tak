...
GP.ui = (function () {
  ...
  function finance(g) {
    ...
    h += '<div class="fin-row total"><span>合計</span><b>' + money(f.weekly) + '万 / 週</b></div>' +
      ...
      ((g.repairPaid || 0) > 0
        ? '<div class="fin-row"><span>🔧 今季ここまでの修理費</span><b class="bad">-' +
          money(g.repairPaid) + '</b></div>' : '') +
  ...
  function modal(title, body, buttons, opts) {
    ...
    /* ---- ✕ は、画面を開くたびに素の状態へ戻す ----
       いくつかの画面は「✕＝この選択」と割り当てて、
       選び終わったら元に戻す約束になっていた。
       ところがボタンで選んだときは戻し忘れていて、
       その手がそのまま次の画面へ持ち越されていた。
       乗っ取られた先（シーズン終了・来季へ・サマーブレイクの結果）では、
       ✕ を押しても関係のない昔の処理が走るだけで、画面は閉じなかった。
       割り当てたい画面は U.modal のあとで付け直す（順番はそのまま効く） */
    const x = $('modalClose');
    if (x) {
      x.onclick = closeModal;
      // 閉じさせない場面は、✕ そのものを出さない（押せるのに効かない、をなくす）
      x.style.display = opts.noClose ? 'none' : '';
    }
